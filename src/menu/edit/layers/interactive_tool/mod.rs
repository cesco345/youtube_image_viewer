// src/menu/edit/layers/interactive_tool/mod.rs
use fltk::{
...
pub fn start_interactive_layer(
    ...
    // Initialize crop selection for layer area
    ...
    let draw_callback = {
        ...
        let outline_color = Color::from_rgb(color.0, color.1, color.2);
        ...
        move |f: &mut Frame| {
            if let Some(img) = &original_image {
                ...
                if let Some((true, (x, y, w, h))) = dimensions {
                    draw::set_draw_color(outline_color);
                ...
                Event::Released => {
                    let should_apply = {
                        ...
                        if let Some((_, _, w, h)) = dimensions {
                            w > 5 && h > 5 && choice2(300, 300, 
                                "Apply color to selected area?", "Yes", "No", "") == Some(0)
                        } else {
                            false
                        }
                    };
                    ...
                    if should_apply {
                        if let Ok(mut state) = state_clone.try_borrow_mut() {
                            if let (Some(selection), Some(current_image)) = 
                                ...
                                if let Ok(Some(new_image)) = 
                                    state.filter_state.apply_filter(current_image, &filter) {
